...
static bool transform_subtraction(Expression* const expression);
...
static bool expression_equal(const Expression* const lhs, const Expression* const rhs);

void simplify_expression(Expression* const expression) {
    assert(expression != NULL);
    transform_subtraction(expression);
...
double evaluate_expression(const Expression* const expression) {
    ...
    switch (expression->type) {
        case ExpressionType_Literal: {
            ...
            if (literal->tag == LiteralTag_Number) return literal->number;
        } break;

        case ExpressionType_Unary: {
            UnaryExpression* const unary = (UnaryExpression*)expression;

            if (unary->operator== TokenType_Minus) return -evaluate_expression(unary->subexpression);
        ...
        case ExpressionType_Binary: {
            ...
            switch (binary->operator) {
                case TokenType_Plus:
                    return evaluate_expression(binary->left) + evaluate_expression(binary->right);
                ...
                case TokenType_Exponent:
                    return pow(evaluate_expression(binary->left), evaluate_expression(binary->right));
            }
        } break;
    }
...
static bool factor_difference_of_squares(Expression* const expression) {
    ...
    switch (expression->type) {
        ...
        case ExpressionType_Binary: {
            ...
            const bool other_result =
                factor_difference_of_squares(binary->left) | factor_difference_of_squares(binary->right);
            ...
            if (binary->operator== TokenType_Minus) {
                ...
                if (lhs_pow_of_two && rhs_pow_of_two) {
                    ...
                    BinaryExpression* new_lhs =
                        expression_binary_create(TokenType_Minus, expression_copy(a), expression_copy(b));
                    // A - B subexpression must be parenthesised
                    new_lhs->base.parenthesised = true;

                    // Create new (A + B) expression, notice the copy
                    BinaryExpression* new_rhs =
                        expression_binary_create(TokenType_Plus, expression_copy(a), expression_copy(b));
                    // A + B subexpression must be parenthesised
...
static bool fold_multipliers_to_diff_of_squares(Expression* const expression) {
    ...
    switch (expression->type) {
        ...
        case ExpressionType_Binary: {
            ...
            if (binary->operator!= TokenType_Multiply) return other_result;
            ...
            if (lhs->operator!= TokenType_Minus || rhs->operator!= TokenType_Plus) {
                return other_result;
            }

            // Left and right factors must be parenthesised
            if (!lhs->base.parenthesised || !rhs->base.parenthesised) return other_result;

            // Subexpressions of factors must be the same
            if (!expression_equal(lhs->left, rhs->left) || !expression_equal(lhs->right, rhs->right)) {
                return other_result;
            }

            binary->operator= TokenType_Minus;

            BinaryExpression* const new_lhs =
                expression_binary_create(TokenType_Exponent, expression_copy(lhs->left),
                                         (Expression*)expression_literal_create_number(2));

            BinaryExpression* const new_rhs =
                expression_binary_create(TokenType_Exponent, expression_copy(lhs->right),
                                         (Expression*)expression_literal_create_number(2));
...
static Expression* expression_copy(const Expression* const expression) {
    ...
    switch (expression->type) {
        ...
        case ExpressionType_Unary: {
            UnaryExpression* const unary = (UnaryExpression*)expression;
            result =
                (Expression*)expression_unary_create(unary->operator, expression_copy(unary->subexpression));
        } break;

        case ExpressionType_Binary: {
            BinaryExpression* const binary = (BinaryExpression*)expression;
            result = (Expression*)expression_binary_create(binary->operator, expression_copy(binary->left),
...
static bool expression_equal(const Expression* const lhs, const Expression* const rhs) {
    ...
    switch (lhs->type) {
        case ExpressionType_Literal: {
            ...
            if (lhs_literal->tag != rhs_literal->tag) return false;

            switch (lhs_literal->tag) {
                case LiteralTag_Number:
                    if (fabs(lhs_literal->number - rhs_literal->number) < DBL_EPSILON) return true;
                    break;

                case LiteralTag_Symbol:
                    if (string_equal(&lhs_literal->symbol, &rhs_literal->symbol)) return true;
                    break;
            }
        } break;
        ...
        case ExpressionType_Unary: {
            ...
            if (lhs_unary->operator!= rhs_unary->operator) return false;
        ...
        case ExpressionType_Binary: {
            ...
            if (lhs_binary->operator!= rhs_binary->operator) return false;
...
static bool transform_subtraction(Expression* expression) {
    assert(expression != NULL);
    bool changed = false;

    if (expression->type == ExpressionType_Binary) {
        BinaryExpression* binary_expr = (BinaryExpression*)expression;

        bool left_changed = transform_subtraction(binary_expr->left);
        bool right_changed = transform_subtraction(binary_expr->right);
        changed = left_changed || right_changed;

        if (binary_expr->left->type == ExpressionType_Literal &&
            binary_expr->right->type == ExpressionType_Literal) {
            Literal* left_literal = (Literal*)binary_expr->left;
            Literal* right_literal = (Literal*)binary_expr->right;

            if (left_literal->tag == LiteralTag_Number && right_literal->tag == LiteralTag_Number) {
                double result;
                if (binary_expr->operator== TokenType_Plus) {
                    result = left_literal->number + right_literal->number;
                } else if (binary_expr->operator== TokenType_Minus) {
                    result = left_literal->number - right_literal->number;
                } else {
                    return changed;
                }

                expression_destroy(&binary_expr->left);
                expression_destroy(&binary_expr->right);
                Literal* new_literal = expression_literal_create_number(result);
                *binary_expr = *((BinaryExpression*)new_literal);
    ...
    if (expression->type == ExpressionType_Unary) {
        UnaryExpression* unary_expr = (UnaryExpression*)expression;
        changed = transform_subtraction(unary_expr->subexpression);
    }
